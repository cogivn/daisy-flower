...
export function BentoItem({ children, className }: WithChildren & { className?: string }) {
  return (
    <div
      className={cn(
        '@container isolate flex flex-col gap-2 overflow-hidden rounded-2xl bg-white p-2 outline outline-gray-950/5 dark:bg-gray-950 dark:outline-white/10 transform-gpu transition-transform duration-300 hover:scale-[1.01]',
        className,
      )}
    >
      {children}
...
export function BentoHeader({ children }: WithChildren) {
  return (
    <>
...
export function BentoTitle({ children }: WithChildren) {
  return (
...
export function BentoDescription({ children }: WithChildren) {
